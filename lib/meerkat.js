...
function getBroadcastsList(cb) {
  request('https://resources.meerkatapp.co/broadcasts?v=1.0',
    function(error, response, broadcasts) {
      return cb(error, JSON.parse(broadcasts).result);
    }
  );
}
...
function getBroadcastsLocation(broadcasts, client, cb) {
...
export function getData(client, next) {
  ...
  async.waterfall([
    ...
    function (broadcasts, cb) {
      getBroadcastsLocation(broadcasts, client, cb);
    },
  ], function (err, result) {
    if (err) {
